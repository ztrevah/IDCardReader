...
import com.example.id_card_reader.RetrofitClient;
...
import com.example.id_card_reader.services.api.LogApiService;
...
public class MainActivity extends AppCompatActivity {
    ...
    private void readNfcTag(Tag tag) {
        readNfcExecutor.execute(() -> {
            ...
            if (isoDep == null) {
                ...
                readNfcResultHandler.post(() -> Toast.makeText(MainActivity.this, "Error reading NFC tag", Toast.LENGTH_SHORT).show());
                return;
            }
                                            ...
                                            if (distance <= 1.0f) {
                                                ...
                                                if(isSendingLog.compareAndSet(false, true)) {
                                                    AddLog addLog = new AddLog(
                                                            UUID.fromString("24CEE65C-F14F-4B57-AA28-F8D48C86590F"),
                                                            mrzInfo.getIdNumber(),
                                                            imageBitmap
                                                    );
                                                    sendLog(addLog);
    ...
    private void sendLog(AddLog addLog) {
        ...
        try {
            ...
            RequestBody imageRequestBody = RequestBody.create(MediaType.parse("image/jpeg"), imageBytes); // Adjust MIME type if using PNG
            ...
            RequestBody deviceIdPart = RequestBody.create(MediaType.parse("text/plain"), deviceId.toString());
            RequestBody civilianIdPart = RequestBody.create(MediaType.parse("text/plain"), civilianId);

            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'", Locale.US);
            String createdAtString = sdf.format(createdAt);
            RequestBody createdAtPart = RequestBody.create(MediaType.parse("text/plain"), createdAtString);

            LogApiService apiService = RetrofitClient.getClient().create(LogApiService.class);
            ...
            call.enqueue(new Callback<ResponseBody>() {
                ...
                public void onFailure(Call<ResponseBody> call, Throwable t) {
                    Toast.makeText(MainActivity.this, "Device error", Toast.LENGTH_LONG).show();
